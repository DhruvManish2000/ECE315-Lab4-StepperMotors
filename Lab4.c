...
static void _Task_Motor( void *pvParameters ){
	...
	// There is a queue to be read from here
	...
	while(1){
		...
		for (int i = 0; i<SEQUENCE_LENGTH; i++){
			...
			xil_printf(" <%d %d> ", positionSequence[i][0], positionSequence[i][1]);
...
static void _Task_Emerg_Stop( void *pvParameters ){
	...
	while(1){
		...
		if(pressedCount >= NO_OF_POLLS){

			/**********************************************************************************************/
			//Set the "current stepper position" to the position at which it must now begin decelerating.
			//There is a stepper driver function for adjusting the current position in steps and you have used it before inside the _Task_Motor().
